	...
	*/
    $.fn.treeTable = function (opts) {
		...
		*/
        ...
        $treeTable.click(function (event) {//alert(0)
        //$(document).on('click', '.tree_table', function (event) {
        ...
        function initRelation($trs, hideLevel) {
            ...
            $trs.each(function (i) {
                if (!this.id) { return; }
                var $tr = $(this);
				var pMapVal = pMap[this.id];
                
                pMapVal && $tr.attr('hasChild', true);
			...
			function reorderTr(id) {
				
				//setTimeout(function(){
					
					if (objPr[id]) {
						
						$.each(objPr[id], function(idx, ele){
							$tbody.append(ele);
						});
						...
						if (id == 0) {
							...
							//console.log($('#' + id, $tbl));
							//$('#' + id, $tbl).after($tbody.find('tr'));
						}
						
						$.each(pMap[id], function(i, v){
			...
			$treeTable.empty();
			$treeTable.append($tbl.find('tbody'));
			//console.log($tbl)
			//$treeTable.replaceWith($tbl)
        }
